...
from utils import normalize_torch, get_BALD_acquisition
...
class BBGDC(nn.Module):
    ...
    def get_weight(self, num_samps, training, samp_type='rel_ber'):
        ...
        if training:
            ...
            # z = p_z.rsample(torch.Size([10000000]))
        else:
...
class BBGDCGCN(nn.Module):
    ...
    def forward(self, train_edge, fix_idx, x, labels, adj, obs_idx, warm_up, adj_normt, training=True
        ...
        if train_edge:
            for i in range(self.nlay):
                ...
                if i == 0:
                    ...
                    if mul_type == 'norm_sec':
                        ...
                        adj_lay = normalize_torch(adj + torch.eye(adj.shape[0]).cuda())
                    elif mul_type == 'norm_first':
                        # adj_lay = torch.mul(mask_mat, adj_normt).cuda()
                        adj_lay = adj_normt.cuda()
                        ...
                        if i < (self.nlay - 1):
                            ...
                            if j == 0:
                                x_out = self.gcs[str((i - 1) * self.nblock + j + 1)](
                                    x[:, j * feat_pblock:(j + 1) * feat_pblock], adj_lay)
                                embedding = x_out
                            else:
            ...
            for i in range(self.nlay):
                ...
                if i==0:
                    # mask_mat = torch.reshape(mask_vec[:self.num_edges], (self.num_nodes, self.num_nodes)).cuda()
                    mask_mat_adj_copy = adj_normt.clone().detach()
                    mask_mat_adj_copy[mask_mat_adj_copy != 0] = mask_vec.reshape(-1)[:self.num_edges]
                    mask_mat = mask_mat_adj_copy
                    if mul_type=='norm_sec':
                        adj_lay = normalize_torch(torch.mul(mask_mat, adj) + torch.eye(adj.shape[0]).cuda())
                        # adj_lay = normalize_torch(adj + torch.eye(adj.shape[0]).cuda())
                    elif mul_type=='norm_first':
                    ...
                    for j in range(self.nblock):
                        ...
                        mask_mat_adj_copy = adj_normt.clone().detach()
                        mask_mat_adj_copy[mask_mat_adj_copy != 0] = mask_vec[j * self.num_edges:(j + 1) * self.num_edges].reshape(-1)
                        mask_mat = mask_mat_adj_copy
        ...
        for x in fix_idx:
            ...
            obs_idx.remove(x)
        obs_idx = torch.from_numpy(np.array(obs_idx)).long().cuda()

        # output = F.softmax(out, dim=1)

        output = F.log_softmax(out, dim=1)
        output_2 = F.softmax(out, dim=1)
        nll_loss_1 = self.loss(labels, output, fix_idx)
        nll_loss_2 = self.loss(labels, output, obs_idx)
        if torch.isnan(nll_loss_2):
            nll_loss = nll_loss_1
        else:
            nll_loss = nll_loss_1+0.8*nll_loss_2
        ...
        return output, output_2, embedding, tot_loss, nll_loss, kld_loss, drop_rates
