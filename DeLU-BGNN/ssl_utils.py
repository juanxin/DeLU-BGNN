...
class ICAContextLabel(Base):
    ...
    def __init__(self, adj, features, labels, nhid, nclass, device, idx_train, args, regression=False):
        self.adj = adj
        try:
            self.sp_features = sp.csr_matrix(features.cpu().numpy())
        except:
            self.sp_features = to_scipy(features)
        self.features = features.to(device)
        self.labels = encode_onehot(labels.cpu())
        self.args = args
    ...
    def classification_loss(self, embeddings):
        ...
        if self.verbose:
            print("ICA label propagate loss:{}".format(loss))
            from metric import accuracy
            ...
            print("ICA label propagate acc:{}".format(acc))
        return loss
